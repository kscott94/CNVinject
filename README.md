...
CNVinject was developed and tested with bwa-mem alignments of ~150 bp Illumina paired-end reads. CNVinject parses alignments via query of qnames and retaines both singleton and paired reads as a feature. CNVinject is compatible with both paired-end and single-end read workflows.
...
--copy-number 3      → subsamples doner bams to increase coverage by 50%    → tandem duplication
--copy-number 3.5    → subsamples doner bams to increase coverage by 75%    → mosaic tandem duplication
--copy-number 4      → subsamples doner bams to increase coverage by 100%   → tandem duplication
```
...
| `-s`, `--seed` | No | none | Random seed. If none, a runtime seed with be randomly generated and printed to consol. |
